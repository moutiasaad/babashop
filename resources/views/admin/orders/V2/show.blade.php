    ...
    <main class="main" id="google_translate_element">
        ...
        <section class="mb_16 order_details_section">
            <div class="container-fluid px-0">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-sm-4">
                            <div class="fs_14 text-sm-end">
                                ...
                                <div class="mb-2">
                                 ...
                                 @if($order->getRawOriginal('status') == 2 &&  $order->is_shipped != 1)
                                <a href="#" class="btn btn-success btn-sm fs_10" data-bs-toggle="modal" data-bs-target="#barq_confirmation">
                                    <i class="fi fi-rr-truck"></i> تاكيد الشحن عبر تطبيق المندوب
                                </a>
                                @endif
                        ...
                        <div class="col-sm-4">
                            <div class=" text-sm-right">
                                ...
                                <div class="">{{ $order->order_type == 0 ? 'لنفسي' : 'إهداء لشخص' }}</div>
                            </div>
                        </div>
                        <div class="col-sm-4">
                                ...
                                <tr>
                                    <td>
                                        <a class="d-flex align-items-center" href="#">
                                            <div class="pe-2">
                                                <img class="tbl_prdImg" src="{{ $line->product->image[0] ?? '#' }}" alt="product" width="64">
                                            </div>
                                            <div class="fs_12">{{ $line->product->name }}</div>
                                        </a>
                                    </td>
                                    <td>{{ $line->qty }}</td>
                                    <td>{{ $line->unit_price }} SAR</td>
                                    <td>{{ $line->product->sku ?? '-' }}</td>
                                    <td>{{ number_format($line->unit_price * $line->qty, 2) }} SAR</td>
                                    <td>
                                        @php $card = json_decode($line->card_description, true); @endphp
                                        @if (!empty($card['from']) || !empty($card['to']) || !empty($card['message']))
                                            <strong>كرت الإهداء:</strong><br>
                                            من: {{ $card['from'] ?? '-' }}<br>
                                            إلى: {{ $card['to'] ?? '-' }}<br>
                                            رسالة: {{ $card['message'] ?? '-' }}
                                        @else
                ...
                @if($order->order_lines->whereNotNull('card_description')->isNotEmpty())
                <div class="bg-white rounded_16 p-3 mt-3">
                    <div class="py_12 px_16">
                        <div class="row align-items-center gy_16">
                            <div class="col-md-6">
                            ...
                            <tbody>
                                @foreach($order->order_lines as $line)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if(!empty($card['link']))
                                                <a href="{{ $card['link'] }}" target="_blank">فتح الرابط</a>
                                            @else
                                                -
                                            @endif
                                        </td>
...
<option value="2">جاري تجهيز هديتك</option>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <form class="w-100" action="{{route('admin.order.confirmBarq',$order->id)}}"  method="POST">
                ...
                <div class="modal-content border-0 rounded_6">
                    <div class="modal-header bg_light2 py_12 px_16">
                        <div class="pe-1">
                           تاكيد التوصيل في تطبيق المندوب لوفارد 
                        </div>
                        <div>
                            <button type="reset" class="btn close_btn" data-bs-dismiss="modal" aria-label="Close">
                                <i class="fi fi-rr-cross-small"></i>
                        ...
                        <div class="col-12">
                            <div class="input-group">
                                <div class="flex-grow-1 row">
                                    <div class="col-12 ">
                                        <div>
                                            <h2 class="form-label fs_14 c_dark2"> سيتم تأكيد إضافة طلب الشحن وتسجيل بيانات الطلب في تطبيق المندوب لوفارد</h2>
                                        </div>
                                    </div>
                                </div>
                        </div>
