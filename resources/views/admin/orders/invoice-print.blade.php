            ...
            <tr>
              ...
              <td class="w-1/2 align-top text-left" style="text-align: left;">
                <div class="text-sm text-neutral-600">
                  ...
                  <p>الاسم: {{ $order->client->fullname ?? '-' }}</p>
                  <p>رقم العميل: {{ $order->client->phone ?? '-' }}</p>
        ...
        <table class="w-full border-collapse border-spacing-0">
          ...
          <tbody>
            ...
            @foreach($order->order_lines as $index => $line)
              <tr>
                <td class="border-b py-3 pr-3">{{ $index + 1 }}</td>
                <td class="border-b py-3 pr-2">{{ $line->product->name ?? '-' }}</td>
                ...
                <td class="border-b py-3 pr-2 text-center">{{ $line->qty }}</td>
                <td class="border-b py-3 pr-2 text-left">
                  <span>
                    {{ number_format($line->unit_price * $line->qty, 2) }}
                    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/98/Saudi_Riyal_Symbol.svg/768px-Saudi_Riyal_Symbol.svg.png" class="currency-icon" alt="﷼">
                  </span>
                </td>
              </tr>
              @php $total += $line->unit_price * $line->quantity; @endphp
            @endforeach
                          ...
                          <tbody>
                            <tr>
                              <td class="border-b p-3 text-right">
                                <div class="whitespace-nowrap text-slate-400">رسوم التوصيل:</div>
                              </td>
                            ...
                            <tr>
                              ...
                              <td class="border-b p-3 text-left">
                                <div class="total-price font-bold text-main">
                                  <span>{{ number_format($order->total_net_a_pay, 2) }}</span>
                                  <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/98/Saudi_Riyal_Symbol.svg/768px-Saudi_Riyal_Symbol.svg.png" alt="﷼">
                                </div>
                              </td>
                            </tr>
