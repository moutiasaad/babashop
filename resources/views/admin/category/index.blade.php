...
<body>
                                ...
                                <table class="table table-hover def_table mb-0">
                                    <thead>
                                        <tr>
                                            <th>#</th> <!-- Image -->
                                            <th>صورة</th> <!-- Image -->
                                            <th>الإسم</th> <!-- Type Merchant ID -->
                                            <th>حالة الظهور</th> <!-- Type Merchant ID -->
    ...
    <script>
        ...
        function showVisibilityModal(productId) {
            ...
            $('.visibility__title').text('هل أنت متأكد أنك تريد تغيير حالة ظهور القسم ؟');
            $('.visibility__desc').text('سيتم تغيير حالة المنتج رقم ');
        }
        ...
        $('.visibility__submitBtn').on('click', function() {
            visibility_update(productIdToDelete);
        });

        function visibility_update(productId) {
            // Redirect to the delete route
            window.location.href = '/dashboard/{{ $Model_API_ROUTE }}/' + productId + '/visiblity';
        }
                        ...
                        tableBody.append(`
                            <tr>
                               <td>${model.order_item}</td>
                                <td><img src="${model.image}" alt="${model.name}" style="width: 50px; height: auto;"></td>
                                <td>${model.name}</td>
                                <td> ${model.visibility == 1 ? 'ظاهر' : 'محجوب'}</td>
                                ...
                                <button 
                                        ...
                                        onclick="showVisibilityModal(${model.id}, ${model.visibility})">
                                        ${model.visibility == 1 ? 'حجب' : 'إظهار'}
                                </button>

                                <a href="/dashboard/product/0?category_id=${model.id}"
