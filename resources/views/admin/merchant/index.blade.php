...
<body>
                            ...
                            <div id="countryTableContainer" class="overflow-x-auto">
                                <table class="table table-hover def_table mb-0">
                                    <thead>
                                        <tr>
                                            <th>#</th> <!-- Brand Name -->
                                            <th>اسم العلامة التجارية</th> <!-- Brand Name -->
                                            <th>صورة</th> <!-- Image -->
                                            <th>نوع التاجر</th> <!-- Type Merchant ID -->
                                            <th>المدينة</th> <!-- Open and Close Hours -->
                                            <th>ساعات العمل</th> <!-- Open and Close Hours -->
                                            <th>تخصيص</th> <!-- Enable Customization -->
                                            <th>نسبة المنصة</th> <!-- Enable Customization -->
                                            <th>تاريخ الانضمام</th> <!-- Created At -->
    ...
    <script>
        ...
        function showDestoryModal(productId) {
            ...
            $('.alert__title').text('هل أنت متأكد أنك تريد حذف المتجر ؟');
            $('.alert__desc').text('سيتم حذف المتجر و جميع منتجاته بشكل نهائي و لا يمكن إلغاء هذه العملية ');
        }
    ...
    <script>
        $(document).ready(function() {
            ...
            function renderTable(models) {
                ...
                if (models.length === 0) {
                    ...
                    tableBody.append(`
                        <tr>
                            <td colspan="8" class="text-center py-5">
                                ...
                                <p class="fs_18 text-gray">لا توجد متاجر لعرضها</p>
                            </td>
                        </tr>
                    `);
                } else {
                    models.forEach(model => {
                        ...
                        tableBody.append(`
                        <tr>
                            <td>${model.order_item}</td>
                            <td>
                                ${model.brand_name}
                                <div class="fs_12 c_gray3"><span>${model.formatted_categories}</span></div>
                            </td>
                            <td><img src="${model.image}" alt="${model.brand_name}" style="width: 50px; height: auto;"></td>
                            <td>${model.type_merchant_id === 1 ? 'محل تجار' : model.type_merchant_id === 2 ? 'متجر إلكتروني' : ''}</td>
                            <td>${model.region?.name}</td>
                            <td>${model.open_at} - ${model.close_at}</td>
                            <td>${model.enable_customization ? 'نعم' : 'لا'}</td>
                            <td>${model.platform_fee}%</td>
            ...
            onclick="showDestoryModal(${model.id})">حذف
                ...
                <a href="/dashboard/product/0?merchant_id=${model.id}"
            class="btn btn_sm btn_danger" >عرض المنتجات</a>
                            </td>                         
                        </tr>
                    `);
                    });
