...
<head>
    ...
    <title>Shaieb Store - Boutiques</title>
...
<body>
                        ...
                        <div class="bg-white rounded_16 pb_12">
                            ...
                            <div class="border-bottom py_12 px_16">
                                <div class="row align-items-center gy_16">
                                    <div class="col-md-5">
                                        <h2 class="fs_18 fw-normal mb-0">Boutiques</h2>
                            ...
                            <div id="merchantTableContainer" class="overflow-x-auto">
                                <table class="table table-hover def_table mb-0">
                                    ...
                                    <tbody id="merchantTableBody">
                                        <tr>
                                            <td colspan="8" style="margin-top:100px;">
    ...
    <script>
        let merchantIdToAction = null;

        function showDeleteModal(merchantId) {
            merchantIdToAction = merchantId;
        ...
        function showVisibilityModal(merchantId) {
            merchantIdToAction = merchantId;
            $('#visibility__modal').modal('show');
            $('.visibility__title').text('Êtes-vous sûr de vouloir changer la visibilité?');
            $('.visibility__desc').text('La visibilité de la boutique sera modifiée.');
        }

        // Delete the merchant when confirmed
        $('.alert__submitBtn').on('click', function() {
            delete_merchant(merchantIdToAction);
        });

        $('.visibility__submitBtn').on('click', function() {
            visibility_update(merchantIdToAction);
        });

        function visibility_update(merchantId) {
            window.location.href = '/dashboard/merchants/' + merchantId + '/visibility';
        }

        function delete_merchant(merchantId) {
            window.location.href = '/dashboard/merchants/' + merchantId + '/delete';
        }
    </script>
    ...
    <script>
        $(document).ready(function() {
            ...
            function renderTable(merchants) {
                let tableBody = $('#merchantTableBody');
                tableBody.empty();
                ...
                if (merchants.length === 0) {
                    tableBody.append(`
                        <tr>
                            <td colspan="8" class="text-center py-5">
                                ...
                                <p class="fs_18 text-gray">Aucune boutique à afficher</p>
                            </td>
                        </tr>
                    `);
                } else {
                    merchants.forEach(merchant => {
                        ...
                        let imageUrl = merchant.image;
                        if (imageUrl && imageUrl.includes('{{env("FILES_CDN")}}')) {
                            imageUrl = imageUrl.replace('{{env("FILES_CDN")}}', '');
                        }

                        // Get type name (simplified)
                        let typeName = merchant.type_merchant_id == 1 ? 'Restaurant' : 'Boutique';

                        tableBody.append(`
                            <tr>
                                <td>${merchant.order_item || '-'}</td>
                                <td>${merchant.brand_name}</td>
                                <td>${typeName}</td>
                                <td>${merchant.open_at} - ${merchant.close_at}</td>
                                <td>${merchant.visibility == 1 ? '<span class="badge bg-success">Visible</span>' : '<span class="badge bg-secondary">Masquée</span>'}</td>
                                <td>${new Date(merchant.created_at).toLocaleDateString('fr-FR')}</td>
                                <td>
                                    <a href="/dashboard/merchants/${merchant.id}/edit" class="btn btn_sm btn_primary">Modifier</a>
                                    <button class="btn btn_sm btn-danger" onclick="showDeleteModal(${merchant.id})">Supprimer</button>
                                    <button
                                        class="btn btn_sm ${merchant.visibility == 1 ? 'btn-warning' : 'btn-success'}"
                                        onclick="showVisibilityModal(${merchant.id})">
                                        ${merchant.visibility == 1 ? 'Masquer' : 'Afficher'}
                                    </button>
                                </td>
                            </tr>
                        `);
