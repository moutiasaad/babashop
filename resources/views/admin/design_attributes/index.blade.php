...
models.forEach(model => {
    console.log(model);
    const isColorType = model.type === 'color_tape' || model.type === 'color_cover';
    tableBody.append(`
        <tr>
            <td>
                ${isColorType ? `<div style="width: 50px; height: 20px; background-color: ${model.title}; border: 1px solid #000;"></div>` 
                              : `<img src="/${model.img}" alt="${model.name}" style="width: 50px; height: auto;">`}
            </td>
            <td>${isColorType ? model.title : model.title}</td>
            <td>${new Date(model.created_at).toLocaleDateString()}</td>
            <td>
                <a href="/dashboard/design-attributes/${model.id}/edit" class="btn btn_sm btn_primary">تعديل</a>
    ...
    <script>
        function delete_product(productId) {
            ...
            window.location.href = '/dashboard/design-attributes/' + productId + '/delete';
        }
    </script>
